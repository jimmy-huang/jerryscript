...
jerry_debugger_receive (jerry_debugger_uint8_data_t **message_data_p) /**< [out] data received from client */
{
  ...
  while (true)
  {
    ...
    /**
     * Work-around Zephyr bug where non-blocking mode on socket is not working, so instead of
     * looping around and call recv(), return instead so debugger can be resumed or else
     * it will be stuck on calling recv().
     */
    if (resume_exec)
    {
      return true;
