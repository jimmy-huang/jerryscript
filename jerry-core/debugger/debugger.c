...
{
  jerry_debugger_init_ws (JERRY_CONTEXT (debugger_port));
  ...
  if (!jerry_debugger_accept_connection_ws ())
  {
    return false;
  }
...
jerry_debugger_send (size_t data_size) /**< data size */
{
  ...
  if (!jerry_debugger_send_ws (JERRY_CONTEXT (debugger_send_buffer),
                               data_size))
  {
    jerry_debugger_close_connection ();
    return false;
...
jerry_debugger_receive (jerry_debugger_uint8_data_t **message_data_p) /**< [out] data received from client */
{
  ...
  while (true)
  {
    ...
    bool success = jerry_debugger_receive_ws (JERRY_CONTEXT (debugger_receive_buffer),
                                              &message_size,
                                              &offset);
