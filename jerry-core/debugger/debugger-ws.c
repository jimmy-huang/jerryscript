...
/**
 * Initialize the websocket sconfigurations.
 *
 * @return true - if the configurations succeeded
 *         false - otherwise
 */
bool
jerry_debugger_init_ws (uint16_t tcp_port)
{
 ...
 */
bool
jerry_debugger_accept_connection_ws (void)
{
  int server_socket;
 ...
 */
bool
jerry_debugger_send_ws (uint8_t *message_data_p, /**< send data pointer */
 ...
 *
 * @return true - if the data was received successfully from the client,
 *         false - otherwise
 */
bool
jerry_debugger_receive_ws (uint8_t *message_data_p, /**< received data pointer */
                           ...
                           uint32_t *data_offset) /**< [in/out] data buffer offset */
{
  ...
  if (byte_recv < 0)
  {
    if (errno != EWOULDBLOCK)
    {
      jerry_port_log (JERRY_LOG_LEVEL_ERROR, "recv error: %s\n", strerror (errno));
      jerry_debugger_close_connection_tcp (true);
      return false;
    }
